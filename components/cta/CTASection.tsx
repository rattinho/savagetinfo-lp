...
export default function CTASection() {
  ...
  useGSAP(() => {
    if (window.innerWidth >= 768) {
      ...
      gsap.set('#contato #ctacard, #contato #infolist > div', { y: 30, opacity: 0 })
      gsap.timeline({
        scrollTrigger: {
          trigger: "#contato",
          start: "top 82%",
        ...
        .to('#contato #ctacard, #contato #infolist > div', { y: 0, opacity: 1, stagger: 0.15, duration: 0.7, ease: "power2.out" })
    }
  }, [])

  return (
    <section id="contato" className="relative md:mt-[-200vh] z-10 md:min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20 md:pt-27.5 pb-0 px-[clamp(20px,6vw,80px)]">
      ...
      <div
        id="ctacard"
        className="relative w-full max-w-190 rounded-[20px] md:rounded-3xl text-center flex flex-col items-center gap-5 md:gap-6 mb-[clamp(32px,8vh,80px)] border border-[rgba(125,176,255,0.18)]"
        style={{
      ...
      >
        <h2
          ...
          style={{ fontSize: "clamp(1.6rem,4vw,3.2rem)", lineHeight: 1.06 }}
        >
        ...
        <p
          ...
          style={{ fontSize: "clamp(0.875rem,1.5vw,1.08rem)" }}
        >
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2.5 font-body font-bold text-[14px] md:text-[15px] text-white px-6 py-3.5 md:px-6.5 md:py-3.75 rounded-full transition-[filter,transform] duration-200 hover:brightness-110 hover:-translate-y-0.5"
          ...
          <a
            ...
            className="inline-flex items-center gap-2.5 font-body font-semibold text-[14px] md:text-[15px] text-[rgba(255,255,255,0.82)] px-6 py-3.5 md:px-6.5 md:py-3.75 rounded-full bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.14)] transition-[background,border-color] duration-200 hover:bg-[rgba(255,255,255,0.09)] hover:border-[rgba(125,176,255,0.4)]"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-4.25 h-4.25">
      ...
      <div id="infolist" className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-3.5 w-full max-w-190 mb-[clamp(36px,6vh,72px)]">
        {INFO_ITEMS.map((item) => (
          <div
            key={item.label}
            className="flex items-center md:items-start gap-3 md:gap-3.5 p-[14px_16px] md:p-[22px_20px] rounded-2xl md:rounded-2xl bg-[rgba(255,255,255,0.025)] border border-[rgba(125,176,255,0.1)]"
          >
            <div className="shrink-0 w-8 h-8 md:w-9 md:h-9 rounded-[9px] md:rounded-[10px] bg-[rgba(46,123,255,0.1)] border border-[rgba(125,176,255,0.18)] flex items-center justify-center text-[rgba(125,176,255,0.85)]">
              {item.icon}
            </div>
            <div className="flex flex-col gap-0.5 md:gap-1">
              <span className="font-display font-semibold text-[12.5px] md:text-[13px] text-white">{item.label}</span>
              <span className="font-body text-[12px] md:text-[12.5px] leading-normal md:leading-normal text-[rgba(255,255,255,0.48)]">{item.content}</span>
            </div>
          </div>
        ))}
      </div>
